...
var $author$project$Main$TimeDeltaReceived = function (a) {
	return {$: 1, a: a};
};
...
var $author$project$Main$parseControlMsg = function (msg) {
	switch (msg) {
		case 'pause':
		...
		case 'seek1':
			return $author$project$Main$Seek(1);
		case 'seek10':
			return $author$project$Main$Seek(10);
		case 'seek-1':
			return $author$project$Main$Seek(-1);
		case 'seek-10':
			return $author$project$Main$Seek(-10);
...
var $author$project$Ports$receiveControlMsg = _Platform_incomingPort('receiveControlMsg', $elm$json$Json$Decode$string);
var $author$project$Ports$receiveEditorMsg = _Platform_incomingPort('receiveEditorMsg', $elm$json$Json$Decode$string);
	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				$author$project$Ports$receiveEditorMsg($author$project$Main$Change),
				$author$project$Ports$receiveControlMsg($author$project$Main$parseControlMsg),
				function () {
				if (model.$ === 3) {
					var player = model.a.j;
					if (player.$ === 1) {
						return $elm$browser$Browser$Events$onAnimationFrameDelta($author$project$Main$TimeDeltaReceived);
					} else {
...
var $author$project$Main$playOrPauseId = 'play-or-pause';
var $author$project$Main$blurPlayOrPause = A2(
	$elm$core$Task$attempt,
	$elm$core$Basics$always($author$project$Main$NoOp),
	$elm$browser$Browser$Dom$blur($author$project$Main$playOrPauseId));
var $author$project$Main$framesPerMillisecond = 0.03;
var $elm$core$Basics$modBy = _Basics_modBy;
var $author$project$Main$frameIndexAt = F2(
	function (time, frameCount) {
		return A2(
			$elm$core$Basics$modBy,
			frameCount,
			$elm$core$Basics$floor(time * $author$project$Main$framesPerMillisecond));
	});
...
var $author$project$Main$lookupBestFrame = F2(
	function (frameIndex, frames) {
		var _v0 = A2($elm$core$Dict$get, frameIndex, frames);
		if (!_v0.$) {
			var svgUrl = _v0.a;
			return $elm$core$Maybe$Just(svgUrl);
		} else {
...
var $author$project$Main$processKeyPress = F2(
				...
				A2(
					...
					function (key) {
						switch (key.$) {
							case 18:
								return $elm$core$Maybe$Just(
									$author$project$Main$Seek(-10));
							case 21:
								return $elm$core$Maybe$Just(
									$author$project$Main$Seek(10));
							case 20:
								return $elm$core$Maybe$Just(
									$author$project$Main$Seek(1));
							case 19:
								return $elm$core$Maybe$Just(
									$author$project$Main$Seek(-1));
							case 17:
								if (model.$ === 3) {
									var player = model.a.j;
									if (player.$ === 1) {
										return $elm$core$Maybe$Just($author$project$Main$Pause);
									} else {
										return $elm$core$Maybe$Just($author$project$Main$Play);
									}
								} else {
									return $elm$core$Maybe$Nothing;
								}
					...
					A2(
						...
						_List_fromArray(
							[$ohanhi$keyboard$Keyboard$navigationKey, $ohanhi$keyboard$Keyboard$whitespaceKey]),
						rawKey))));
	});
...
var $author$project$Main$Animating = function (a) {
	return {$: 3, a: a};
};
...
var $author$project$Fps$init = A2($elm$core$List$repeat, 50, 1000);
var $author$project$Main$initAnimation = function (frameCount) {
	return {
		D: $elm$core$Maybe$Nothing,
		z: frameCount,
		E: $author$project$Fps$init,
		p: 0,
		u: $elm$core$Dict$empty,
		j: $author$project$Main$Playing(0),
		G: false
	};
};
				...
				switch (_v0.a) {
					...
					case 'status':
						if (_v0.b.b && (!_v0.b.b.b)) {
							...
							switch (status) {
								case 'Compiling':
									return $author$project$Main$Compiling;
								case 'Done':
									return model;
								default:
								...
								if (model.$ === 3) {
									...
									return $author$project$Main$Animating(
										_Utils_update(
											animation,
											{
												u: A3($elm$core$Dict$insert, frameIndex, svgUrl, animation.u)
											}));
								} else {
									return $author$project$Main$Problem(
										$author$project$Main$UnexpectedMessage('Got \'frame\' message while not Animating'));
								}
							} else {
								return $author$project$Main$Problem(
...
var $author$project$Main$sendSource = function (txt) {
	return A2(
		$bburdette$websocket$WebSocket$send,
		$author$project$Ports$sendSocketCommand,
		$bburdette$websocket$WebSocket$Send(
...
var $author$project$Fps$addToHead = F2(
	function (dt, fps) {
		if (!fps.b) {
			...
			var x = fps.a;
			var xs = fps.b;
			return A2($elm$core$List$cons, x + dt, xs);
		}
	});
...
var $author$project$Fps$update = F3(
	function (hasNewFrame, dt, fps) {
		return A2(
			$elm$core$List$take,
			50,
			hasNewFrame ? A2($elm$core$List$cons, dt, fps) : A2($author$project$Fps$addToHead, dt, fps));
	});
var $author$project$Main$updateAnimation = F2(
	function (f, model) {
		if (model.$ === 3) {
			var animation = model.a;
			return $author$project$Main$Animating(
		...
		switch (msg.$) {
			case 1:
				...
				return _Utils_Tuple2(
					A2(
						$author$project$Main$updateAnimation,
						function (animation) {
							var frameCount = animation.z;
							var frames = animation.u;
							var player = animation.j;
							var frameDeltas = animation.E;
							if (player.$ === 1) {
								var time = player.a;
								var newTime = time + delta;
								var newFrameIndex = A2($author$project$Main$frameIndexAt, newTime, frameCount);
								var hasNewFrame = A2($elm$core$Dict$member, newFrameIndex, frames);
								return _Utils_update(
									animation,
									{
										D: A2($author$project$Main$lookupBestFrame, newFrameIndex, frames),
										E: A3($author$project$Fps$update, hasNewFrame, delta, frameDeltas),
										p: newFrameIndex,
										j: $author$project$Main$Playing(newTime)
									});
							} else {
								return animation;
							}
			...
			case 4:
				return _Utils_Tuple2(
					A2(
						...
						function (animation) {
							var frameCount = animation.z;
							var frameIndex = animation.p;
							var player = animation.j;
							if (!player.$) {
								var newTime = frameIndex / $author$project$Main$framesPerMillisecond;
								return _Utils_update(
									animation,
									{
										p: A2($author$project$Main$frameIndexAt, newTime, frameCount),
										j: $author$project$Main$Playing(newTime)
									});
							} else {
								return animation;
						...
						model),
					$author$project$Main$blurPlayOrPause);
			case 5:
				var delta = msg.a;
				return _Utils_Tuple2(
					A2(
						...
						function (animation) {
							...
							var frames = animation.u;
							if (!player.$) {
								var newFrameIndex = A2($elm$core$Basics$modBy, frameCount, frameIndex + delta);
								return _Utils_update(
									animation,
									{
										D: A2($author$project$Main$lookupBestFrame, newFrameIndex, frames),
			...
			case 6:
				...
				return _Utils_Tuple2(
					model,
					A2($author$project$Main$processKeyPress, rawKey, model));
			case 3:
				return _Utils_Tuple2(
					A2(
						$author$project$Main$updateAnimation,
			...
			default:
				...
				return _Utils_Tuple2(
					model,
					$author$project$Main$sendSource(txt));
		}
	});
...
var $author$project$Main$ToggleHelp = {$: 7};
...
var $author$project$Main$helpModal = function () {
	var explainKey = F2(
		function (key, legend) {
			return A2(
				$elm$html$Html$tr,
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('help-button button'),
				...
				_List_fromArray(
					[
						A2(explainKey, 'SPACEBAR', 'Pause / Play animation'),
						A2(explainKey, 'ARROW LEFT', ' Move back 1 frame'),
						A2(explainKey, 'ARROW RIGHT', ' Move forward 1 frame'),
						A2(explainKey, 'ARROW UP', ' Move forward 10 frames'),
						A2(explainKey, 'ARROW DOWN', ' Move back 10 frames')
					]))
			]));
}();
...
var $author$project$Main$playControls = function (paused) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
			...
			[
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$id($author$project$Main$playOrPauseId)
					]),
				_List_fromArray(
					[
...
var $author$project$Main$frameView = F7(
	function (bestFrame, frameIndex, frameCount, frames, showingHelp, frameDeltas, isPaused) {
		var progressView = function () {
			var receivedFrames = $elm$core$Dict$size(frames);
			return (!_Utils_eq(receivedFrames, frameCount)) ? A2($author$project$Main$progressBar, receivedFrames, frameCount) : $elm$html$Html$text('');
		}();
		var image = function () {
			if (!bestFrame.$) {
		...
		var helpView = showingHelp ? $author$project$Main$helpModal : A2(
			$elm$html$Html$button,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('help-button button'),
		...
		var bar = A2(
			...
			_List_fromArray(
				[
					$author$project$Main$playControls(isPaused),
					A2(
					$elm$html$Html$span,
					_List_Nil,
					_List_fromArray(
						[
							...
							' Frame: ' + (A3(
								...
								digitCount,
								'0',
								$elm$core$String$fromInt(frameIndex + 1)) + (' / ' + frameCountStr)))
						])),
					progressView,
					helpView
				]));
	...
	switch (problem.$) {
		...
		case 1:
			return A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Failed to establish connection to server. Sorry. :-/')
					]));
		case 2:
				...
				_List_fromArray(
					[
						...
						switch (model.$) {
							...
							default:
								var frameCount = model.a.z;
								var frames = model.a.u;
								var frameIndex = model.a.p;
								var player = model.a.j;
